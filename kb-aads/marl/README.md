...
**Update, 2026-09-07: Jury is now trained too, reusing this same dataset.** `train_jury.py` collapses `target_containment` to a binary CONTAIN (any level > 0) / ALLOW (0) target and trains against the identical `train.csv`/`val.csv`/`test.csv` splits — no separate data collection needed, since "should this be contained at all?" is a strict coarsening of Containment's "which level?" question, not a different question. Reward follows the Reward Signals table below (True Positive +1.0 / False Positive -0.5 / True Negative +0.1 / False Negative -1.0), which was written for exactly this agent. Held-out test results: 99.4% overall accuracy, 100% CONTAIN recall, 0% false-negative rate (908 samples). `consensus/jje.py`'s `JuryAgent` loads the resulting checkpoint (`marl/checkpoints/jury_ppo`) the same inference-only way `ContainmentAgent` does, and falls back to the original fixed-threshold `decide_vote` when either no checkpoint exists or a caller only supplies a bare confidence score (not a full obs vector) — every existing caller/test does the latter today, so behavior is unchanged until real full-observation alerts start flowing.
