...
NOMAD (the hardware trust-anchor extension) previously had its design proposals here under `hardware-ext/` — that directory has been removed. NOMAD is now developed in its own separate repository (`github.com/pardhuvarmax/the-nomad`) by a dedicated hardware sub-team, not documented in this repo.
