...
- [`kbc-hardware-watchdog.md`](kbc-hardware-watchdog.md) — **Proposed, not implemented.** Extends `kb-checker`'s existing external-watchdog role to a physically separate SBC (out-of-band link, TPM root of trust, relay-based fencing), addressing the one failure mode software-only watchdogging on the same host can't structurally cover: total kernel compromise. Includes a bench-buildable bill of materials; open questions (wire protocol, failure semantics) explicitly unresolved.
