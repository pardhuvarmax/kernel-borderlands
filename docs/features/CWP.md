...
**Update, 2026-09-07: §9 alerting and §4.3/§11.4 local-signing are now implemented.**
- **§9 severity-escalated alerting**: `internal/controlplane/severity.go`'s `classifySeverity`/`escalateSeverity` give every BORDERLANDS-entry alert a score-derived base tier (LOW/MEDIUM/HIGH/CRITICAL), and `findCWPMatch` escalates it one tier (capped at CRITICAL) whenever the alerting process matches a CWP-registered workload — populating the `Alert` proto's new `protected_workload`/`owner_team`/`justification`/`policy_id` fields (`kb.proto`) and logging/audit-logging a `[CWP] Protected Workload Alert — ESCALATED` line matching §9.3's example format. **Known limitation, stated in `findCWPMatch`'s own doc comment**: this Go-side match is by executable *basename* (`comm`), not the canonical resolved path §5.2/§11.2 require — kbd never receives a process's full path over the wire, only its 16-byte `comm`. The real, path-resolved, spoofing-resistant match still happens kernel-side in `cwp_classify()` for the actual containment-exemption decision (§7-§8); this basename match only decides alert severity/routing, a lower-stakes purpose.
- **§4.3/§11.4 signed local policy**: `internal/ipc/cwp.go`'s `SignWorkloadsFile`/`VerifyWorkloadsSignature` implement Ed25519 signing over `workloads.yaml`'s `critical_workloads` + `policy_metadata` content (excluding the signature field itself). `kbd --workloads-pubkey <path>` makes signature verification mandatory — a missing or invalid signature is rejected and the previous known-good registry is kept, never a partially-trusted one. `kbctl workload keygen`/`sign` give operators an actual signing workflow. **What this does NOT implement**: §4.3's networked "Central Policy Store" push/pull — no fleet-management server exists anywhere in this codebase for any subsystem, and inventing one (transport, auth, discovery) is a separate infrastructure decision. This closes the "per-host signed policy cache, verified before load" half of §4.3, which is the concretely-specified, buildable part; the central-store distribution mechanism remains open.
