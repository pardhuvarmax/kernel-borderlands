...
**SUPERSEDED as of §2.11 (STATUS: DONE).** Everything below this point describes the
custom in-process Go SSH server (`internal/ssh/`) as it existed before the migration — kept
as design-rationale history (the auth-bypass/single-implementation risk analysis below is
part of *why* the migration happened), not as current-state documentation. `internal/ssh/`
has been deleted from `kb-control-plane` in full. **What actually runs now**: a second,
independent `sshd` instance (`sshd@kb-operator.service`) owns port 2222 entirely, with
`ForceCommand /usr/local/bin/kb-tui` — `kbd` has zero SSH code left (no host keys, no
`authorized_keys` parsing, no PTY plumbing). Full unit/config: `docs/architecture/
boot_sequence_spec.md` §3. The "no network exposure" correction two paragraphs down is
still true, verbatim — the port just moved from `kbd`'s process to a dedicated `sshd`'s.
...
#### STATUS : DONE. `SubmitAgentDecision` (`internal/controlplane/grpc.go`) now rejects `TERMINATE`/`NAMESPACE`/`SECCOMP`/`CGROUP` unless `AuthorizedBy` contains at least one identity present in a configured allowlist (`KB_AGENT_ALLOWLIST`, defaulting to this system's known agent roles). Regression tests: `TestSubmitAgentDecision_UnrecognizedAuthorizedByRejectedForDestructiveAction`, `TestSubmitAgentDecision_RecognizedAuthorizedByPassesGate` (`grpc_test.go`).
...
- **Why it matters**: CPM (Critical Process Module — see `docs/features/CPM.md`) is correctly scoped to `kb-core`, and **as of this pass, is now implemented and verified live there** (`docs/features/cpm-implementation.md`) — gating containment writes on the C sensor side (PID 1, kernel threads, self-registered sensor components, a protected-executable registry). But the Go control plane is the *other* entry point through which a containment request can originate (operator RPC, agent decision), and it still has zero exemption logic of its own — `Contain()` will forward a request for PID 1, `systemd`, or `kbd`'s own PID exactly like any other process, with only `kb-core`'s gate (one hop downstream, over `kbct.sock`) standing between that request and the kernel map. This is now the more pressing half of the gap, not the less pressing one: relying on a single gate that *does* exist in `kb-core` means one bug or race there has no backstop, whereas before neither side had a gate at all.
...
#### STATUS : DONE. `Enforcer.Contain` (`internal/enforcement/enforce.go`) now refuses PID 1, `os.Getpid()` (self), and a configurable list of protected comms (`KB_PROTECTED_COMM`) — independent of `kb-core`'s CPM gate. `ContainmentNone` (restore) is exempt from the gate, matching this section's own "downgrade + alert" spirit rather than blocking recovery from a bad prior containment call. Regression tests: `TestContain_RefusesPid1`, `TestContain_RefusesSelfPid`, `TestContain_RefusesProtectedComm`, `TestContain_RestoreExemptFromProtectedPidGate` (`enforce_test.go`).
...
#### STATUS : DONE. Both RPCs now verify the target PID exists in the store before acting (`codes.NotFound`/a `Success:false` ack otherwise) and enforce a per-caller minimum interval (`KB_AGENT_MIN_INTERVAL_MS`, default 200ms) — a token-bucket wasn't needed for the stated threat (a looping buggy/compromised client); a minimum-interval throttle is simpler and covers it. Regression tests: `TestSetContainment_UnknownPidReturnsNotFound`, `TestSetContainment_ThrottlesRepeatedCalls`, `TestSubmitAgentDecision_UnknownPidRejected`, `TestSubmitAgentDecision_ThrottlesRepeatedCallsPerAgent` (`grpc_test.go`).
...
#### STATUS : DONE — was already resolved before this section was last read, this is a doc-staleness fix, not new work. Following the sketch fix's own hint turned up that `KB_WIRE_MSG_PROCESS_EXIT` (`kb-core/userspace/bridge/kb_bridge.h`, value `4`) already exists and is sent (`kb_bridge_send_process_exit`, called from `kbd_sensor.c`'s `KB_EVT_PROCESS_EXIT` case); Go already parses it (`ipc.MsgTypeProcessExit`) and dispatches to `OnProcessExit`, which calls `store.TerminateProcessState` — confirmed that deletes both the L1 `sync.Map` entry and the L2 SQLite row, not just one. `CHANGELOG.md` credits this to "Process Exit Lifecycle (kb-control-plane)" [Tejaswini4119], predating this catalog entry's SEV/Sketch-fix text above, which was simply never updated after the fact. Removed the stale `// TODO: C side should send a dedicated process_exit wire message type.` comment left in `OnProcessState` (`controlplane.go`) describing a gap that no longer exists. Existing regression test: `TestOnProcessExitFlushesCache` (`controlplane_test.go`).
...
#### STATUS : DONE — doc-staleness fix, same situation as §2.5. `kbctl policy reload` → `ReloadPolicy` gRPC handler → `reloadPolicyFromDisk()` already builds a fresh `policy.Engine` and swaps it into `cp.policy` under `policyMu` (a proper `sync.RWMutex`, not the SIGHUP-based approach this section's sketch fix proposed — a live RPC is more useful operationally than a signal, since it works over the same UDS surface `kbctl` already uses for everything else). `BUG_REPORT.md`'s RESOLVED-3 documents this in detail, including that it was specifically traced for a suspected data race and confirmed race-free. This catalog section was simply never updated to match. The sensor-side push-on-reconnect behavior this section correctly scoped out as `kb-core`'s concern remains exactly that — out of scope here, not claimed as done.
...
#### STATUS : DONE. `cmd/kbd/main.go` now has `--http-addr`/`--grpc-socket` cobra flags with real `--help` text, matching `--db`/`--policy`'s existing pattern exactly. `KB_HTTP_BIND`/`KB_GRPC_SOCKET` still work — they set the flag's default at `init()` time, so an unset flag falls back to them, keeping every existing deployment script working unchanged. `ControlPlane.Start`/`Stop` signatures changed from `Start()`/`Stop()` to `Start(httpAddr, grpcSocketPath string)`/`Stop(grpcSocketPath string)` to thread the values through — the only caller was `cmd/kbd/main.go` itself, so no other breakage. Also fixed a related consistency gap while touching this: the dashboard's `/api/services` health probe (`http.go`) previously re-read `KB_GRPC_SOCKET` independently rather than using the same value `Start()` received, so `--grpc-socket` wouldn't have been reflected in the dashboard's own connectivity check — `ControlPlane` now stores `grpcSocketPath` once in `Start()` and `http.go` reads that instead.
...
#### STATUS : DONE — doc-staleness fix, same situation as §2.5/§2.6. `GetProcessState` (`grpc.go`) already returns `status.Errorf(codes.NotFound, "no tracked process with pid=%d", req.Pid)` exactly as the sketch fix proposed; `BUG_REPORT.md`'s RESOLVED-2 documents this. This catalog section was never updated to match. `ListZone` (a server-streaming RPC) doesn't have an equivalent gap by construction — an untracked zone simply streams zero results, which isn't ambiguous the way a unary RPC's zero-value struct is, so nothing was missing there either.
...
#### STATUS : SUPERSEDED — §2.11 has now landed (see its STATUS note). The interim `internal/ssh/{server.go,config.go}` patches described below (per-source-IP throttle, `KB_SSH_ALLOW_DEV_FALLBACK` opt-in) existed only briefly — both this file's code and its regression tests are deleted in full, exactly as predicted when they were added. Auth-attempt throttling and dev-mode-fallback risk are no longer this codebase's problem at all: `sshd` (real OpenSSH) owns both now, with its own decades-old hardening, not a custom Go reimplementation of either.
...
#### STATUS : DONE for items 1 and 4; item 3 landed with one deliberate deviation; items 2 and 5 remain open. `internal/ssh/` (server.go, auth.go, hostkey.go, config.go, session.go, ssh_test.go — all of it) is deleted from `kb-control-plane`; the "not a `kbd`-supervised child process" decision above is implemented exactly as specified. `docs/architecture/boot_sequence_spec.md` §3 has the full `sshd@kb-operator.service` unit + `sshd_config.d/kb-operator.conf` drop-in.
- **Item 1 (separate sshd instance)**: done — dedicated unit, own config, own host key path (`/etc/kb/ssh/ssh_host_ed25519_key`), independent of port 22's `sshd`.
- **Item 2 (certificates over static keys)**: **not done** — `AuthorizedKeysFile /etc/kb/authorized_keys` (a flat file) is used for now, deliberately, since §2.12's `step-ca` CIS work this depends on hasn't been built. Tracked as still-open under §2.12, not silently dropped.
- **Item 3 (systemd sandboxing)**: done with one deviation from the literal checklist — `ProtectSystem=strict` needs `ReadWritePaths=/run/kb` alongside it so `kb-tui` (running as the `ForceCommand` target under this same process tree) can reach `kba.sock`; without it `ProtectSystem=strict` would make the socket unreachable and every session would fail post-auth. Documented inline in the unit file itself.
- **Item 4 (`AllowTcpForwarding no`/`X11Forwarding no`)**: done, in the drop-in config exactly as specified.
- **Item 5 (`operator` unprivileged system user, `kb` group membership)**: **not done as a repo artifact** — this is host-provisioning work (create the user, add to the group), out of scope for anything `kb-control-plane`'s source controls; the drop-in's `Match User operator` block assumes this user exists. Flagged here so it isn't missed at actual deployment time.

Regression coverage: the deleted `internal/ssh/` package's tests (`TestAuthThrottle_*`, `TestLoadConfig_DevMode*`, `TestHostKeyLoadAndPersist`, `TestAuthorizedKeysParsingAndValidation`) are gone with it, per this section's own earlier prediction (§2.10/§2.13 status notes) that they'd need deleting, not porting, once this landed — there's nothing left in Go to unit-test; `sshd`'s own correctness is OpenSSH's, not this repo's, to verify. `kb-control-plane`'s full test suite (`go test ./...`) passes with zero SSH-related tests remaining, and `go.mod` lost the `charmbracelet/*`+`creack/pty` dependency chain entirely (`go mod tidy` confirmed nothing else in the module used them).
...
#### STATUS : item 5 (audit tie-in) DONE; items 1–4 (the actual CIS/`step-ca` platform work) remain open, exactly as this section's own "Owner" note scoped them — standing up `step-ca`, its OIDC provisioner, and distributing `TrustedUserCAKeys`/`AuthorizedPrincipalsFile` across hosts is platform/infra work, not something `kb-control-plane`'s source can complete on its own, and none of it happened here.
- **Item 5, done**: added a narrowly-scoped `RecordSSHSession(SSHSessionEvent) returns (Empty)` RPC (`proto/kb.proto`, `internal/controlplane/grpc.go`) — the "audit-write RPC" this step said it depended on. Deliberately not a generic arbitrary-action write endpoint: it only ever accepts `event ∈ {"session_start", "session_end"}` and writes one of exactly two fixed audit actions (`SSH_SESSION_START`/`SSH_SESSION_END`), rejecting anything else with `codes.InvalidArgument`. Added `kbctl ssh session-start`/`session-end` (`kb-op/kbctl/cmd_ssh.go`, hidden from `--help` — not meant to be run interactively) as the caller, and a `ForceCommand` wrapper script (`docs/architecture/boot_sequence_spec.md` §3, `/usr/local/bin/kb-tui-session-wrapper.sh`) that calls `kbctl ssh session-start`, runs `kb-tui` as a child (not `exec`'d, so the wrapper is still alive to log session-end and propagate the real exit code), then calls `kbctl ssh session-end`. Both `kbctl` subcommands treat an audit-logging failure as best-effort — they print to stderr and still exit 0, so an audit-log outage can never turn into an operator-access denial. Regression tests: `TestRecordSSHSession_SessionStartLogsAudit`, `TestRecordSSHSession_SessionEndLogsAudit`, `TestRecordSSHSession_UnknownEventRejected` (`grpc_test.go`).
- **Items 1–4, still open**: no CA is stood up anywhere, `AuthorizedKeysFile` (flat file) remains the live auth mechanism (§2.11's status note says the same), and the `identity` field `RecordSSHSession`/`kbctl ssh session-start` accept is currently always empty — sshd doesn't expose the auth key's fingerprint to a `ForceCommand` child by any standard mechanism, and there's no cert principal to report yet since certs don't exist. Once items 1–4 land, `AuthorizedPrincipalsFile`/`AuthorizedPrincipalsCommand` output becomes the natural source for `--identity`; the wrapper script and RPC don't need to change shape to accept it, just to start passing a real value.
...
#### STATUS : DONE — resolved as a side effect of §2.11, exactly as this section originally predicted. `internal/ssh/session.go`'s path-guessing logic (and the interim `KB_TUI_PATH`-plus-fixed-path patch that briefly replaced it) is gone entirely — `ForceCommand /usr/local/bin/kb-tui` in `sshd_config.d/kb-operator.conf` (`docs/architecture/boot_sequence_spec.md` §3) is now the single, static, packaged answer to "where is kb-tui," with no runtime guessing of any kind. This *is* this section's own "Option 1," arrived at via §2.11 rather than as standalone work, exactly as predicted.
...
- **CPM (Critical Process Module)** — `docs/features/CPM.md`, scoped to `kb-core` (Component: Kernel Behaviour Sensor). **Implemented and verified live as of this pass** — see `docs/features/cpm-implementation.md`. **Owner**: Pardhu, `kb-core`. Related Go-side backstop is 2.3 above (that piece is Teju's, still unimplemented — see 2.3's updated text), but the primary implementation is his, not control-plane work.
- **CWP (Critical Workload Protection)** — `docs/features/CWP.md`, spans `kb-core` and `kb-control-plane`. **Both sides now implemented.** Kernel/sensor: `protected_workloads_map`/`protected_workload_paths_map`, `cwp_classify()` gated strictly after CPM, path- and hash-tier identity verification, the `KB_WIRE_MSG_CWP_WORKLOADS` wire receiver, the periodic orphan-reconciliation sweep — `kb-core/tests/test_cwp.py`. Go-side sender (closed 2026-08-30, was the one missing piece): `internal/ipc/cwp.go` (`LoadWorkloadsYAML`/`SendCWPWorkloads`), a new `ReloadWorkloads` RPC + `kbctl workload reload` for the live-push path (the sensor polls for this continuously, not just at connect), `--workloads` on `cmd/kbd/main.go`. Regression tests: `internal/ipc/cwp_test.go`, `TestReloadWorkloads_*` (`grpc_test.go`). Still open: owner-team/justification alerting (CWP.md §9's severity escalation) and §4.3's signed cluster-wide policy sync — see `CWP.md`'s own status note. **Owner**: Pardhu, `kb-core` (done); Go-side sender also done, unassigned going forward for the still-open alerting/cluster-sync pieces.
...
- `kbctl` (`kb-op/kbctl/`) is a real Go source tree now, not just a README — `main.go` plus `cmd_*.go` files, correctly targeting `kba.sock` (its README's earlier `:50051` TCP staleness has since been corrected).
