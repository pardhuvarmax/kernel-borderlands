...
- **CWP (Critical Workload Protection)** — `docs/features/CWP.md`, `Status: Design Specification`, scoped to `kb-core`. Confirmed unimplemented anywhere in the repo as of this writing — explicitly depends on CPM running first in the pipeline (CWP.md §8), which is why CPM was implemented first. **Owner**: Pardhu, `kb-core`.
