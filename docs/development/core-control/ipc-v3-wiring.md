...
**Status:** Completed — `WireVersion = 3` in `kb-control-plane/internal/ipc/wire.go` matches `KB_WIRE_VERSION` in `kb-core/userspace/bridge/kb_bridge.h`. Note: the Go-side code samples below (inline `cgroupThrottle`/`sigkill`, TODO stubs for SECCOMP/NAMESPACE) are superseded — current `internal/enforcement/enforce.go` forwards all containment levels generically to the C sensor over the wire instead. **Second correction, added later:** every `/var/run/kbd.sock` reference below (including in the embedded code samples) is doubly stale now — the real default was always `/run/kb/kbd.sock`, not `/var/run/kbd.sock`, and as of the `kbd.sock`/`kbct.sock` split (`docs/development/core-control/control-plane-catalog.md` §5.3), `kbd.sock` is telemetry-only (sensor → Go) while containment commands (what this doc's `SendContainmentCmd`/enforcement content is actually about) now go out over a separate `/run/kb/kbct.sock` connection. Left the embedded historical code samples as-written below rather than editing each one — they're a snapshot of what was actually implemented at v3, not a live spec — but don't copy the socket path or single-connection assumption from them into new work.
