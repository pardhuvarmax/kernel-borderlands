...
**Update, 2026-09-07: implemented on the Go Control Plane, as this doc recommended.**
`internal/detection/exfil.go` implements §3.A (Shannon entropy of timing
intervals) and §3.B (CUSUM) close to verbatim from the blueprint below —
`NetFlowWindow`/`Push`/`CalculateTimingEntropy` are a direct port of §4's
Go example. Wiring: a new userspace-only wire message
(`KB_WIRE_MSG_NET_FLOW`, `kb_bridge.h`/`.c`) sends `{pid, daddr, dport,
ts_ns}` from `kbd_sensor.c`'s existing `KB_EVT_NETWORK_CONNECT` handler
(no eBPF/kernel-side change, per this doc's own "out-of-band" guidance) to
`kbd`, which feeds `internal/controlplane.OnNetFlow` → `detection.Detector`.
An alert (`EXFIL_BEACON_SUSPECTED`) fires only when BOTH entropy is below
threshold AND CUSUM has crossed, debounced to one alert per flow per 5
minutes; auto-NAMESPACE containment is available but opt-in per-comm via
`policy.yaml`'s `auto_namespace_on_exfil_beacon` (off by default — a
statistical heuristic has real false-positive risk).

**Fidelity limitation, carried over into the implementation**: §3.B's
CUSUM example uses byte volume as its input. `connect()` events carry no
payload-size field, and adding real per-byte accounting would need new
`write()`/`sendto()` syscall telemetry that doesn't exist on the wire
today — building that is a separate, larger change (new eBPF tracepoints,
fd-to-socket correlation) and is explicitly out of scope for this pass.
The shipped CUSUM instead runs over **connection frequency per time
bucket** as the volume proxy — it still catches a persistent shift in a
process's connection rate, just at connection-count granularity, not byte
granularity. §3.C's autocorrelation algorithm was not implemented (entropy
+ CUSUM combined already cover the beaconing pattern in this doc's own
worked example; autocorrelation would be an additional refinement, not a
gap in what was asked for here).
