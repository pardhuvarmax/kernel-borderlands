...
## The architecture is documented, but the docs are stale Day-1 drafts — correction

Earlier in this session I stated "RL vs. LLM" was already specified and shouldn't be re-litigated, citing `kb-aads/marl/README.md`, `docs/getting-started/requirements.md`, and `scripts/dataset/README.md`. That was too confident. Checked via `git log --follow`: `marl/README.md` and `scripts/dataset/README.md` have **exactly one commit each — `7bf39f8 "Project Development Initialisation"`, 2026-06-25** — untouched since the very first commit, never revisited as the rest of the architecture evolved (including the Ray-only pivot away from ZeroMQ that happened afterward and *did* get propagated to other docs, just not these). `requirements.md`'s last touch (2026-07-09) was a Kafka/ZeroMQ cleanup commit, not a revalidation of the QLoRA/dataset content cited here. So: the RL-vs-LLM split (RLlib for Jury/Healer/Containment, an LLM for Hunter) still holds — that's a real, sound split. But the *specific shape* of the LLM training data these docs describe is an unrevalidated Day-1 draft, not a settled spec, and one part of it turned out to be wrong on inspection (next section).

So the design is: **RLlib policy → numeric vote/action decisions** (Jury), **fine-tuned LLM → investigative reasoning** (Hunter). That split stands. Everything about *how* the LLM side works needs a fresh look, not a re-litigation of RL-vs-LLM itself.

## Hunter's training data: prompt/completion isn't enough — resolved as agentic

`scripts/dataset/README.md`'s planned format is flat **prompt/completion pairs** — one fixed narrative in, one assessment out (`"Process bash (PID 5678) executed: socket→connect(4444)→dup2→execve(/bin/sh). Score: 79.4..."` → a confidence score + IOC name + recommendation). That's a reasonable shape for a single last-mile "given assembled evidence, produce an assessment" step, but it silently assumes something else already gathered and flattened the evidence into one clean sentence — and `HunterAgent.investigate()`'s own TODOs describe an inherently multi-step process (*query control plane for history → build evidence chain → calculate confidence → submit to jury*), not a single input→output pair.

**Resolved, per Karthik: Hunter should be genuinely agentic** — the model itself decides what to query next, in a loop (ReAct-style: reason → call a tool → observe the result → reason again → conclude), not a single flat completion. This has real, concrete downstream consequences that weren't true under the flat-pair assumption:

- **Phase 0's dataset shape changes**: needs multi-turn *trajectory* data (thought/tool-call/observation sequences ending in a final assessment), not flat prompt/completion pairs. `scripts/dataset/label.py`/`format.py` (still unbuilt) need to produce trajectories, not single-turn examples — this is a materially different labeling task than the original plan.
- **The tools already exist — this isn't hypothetical**: `kb-aads/comms/grpc_client.py`'s `ControlPlaneClient` already has `get_process_state(pid)`, `list_zone(zone)`, `stream_events(...)`, `stream_alerts(...)` — real gRPC calls over `/run/kb/kba.sock`. These are the natural tool surface for Hunter's "query control plane for history" step; no new client code needed, just exposing these as callable tools to the model.
- **Base model choice gains a new criterion**: `marl/README.md`'s three candidates (Phi-3 Mini / Qwen2.5 3B / Mistral 7B) were picked with no stated tool-use criterion. Tool-calling/function-calling capability should now weigh into that choice — Qwen2.5's family has comparatively strong native tool-use support; this is worth Karthik's explicit evaluation, not an assumption any of the three "just works" for agentic tool loops.
- **New open questions this creates**: how many tool-call iterations before Hunter is forced to conclude (unbounded loops risk blowing the latency budget that made an LLM affordable here in the first place — Hunter is triggered per zone-transition, not per tick, but that budget isn't infinite either); how tool outputs get formatted back into the model's context window; and whether inference-time tool-calling uses the fine-tuned model's native function-calling format or a simpler ReAct-style text protocol layered on top.

**Terminology note, to avoid re-litigating this later**: "agentic" and "RL" are answering different questions and shouldn't be conflated. RL-vs-LLM (the per-agent table below) is about *what kind of model decides* — a numeric policy trained on reward signal, vs. a language model producing reasoning. "Agentic" describes *how the LLM operates at inference* — a multi-step tool-call loop instead of one forward pass; it's a control-flow pattern, not a training paradigm. Hunter being agentic does not move it into the RL column — it's still the QLoRA-fine-tuned LLM below, still trained via supervised fine-tuning (imitation learning on labeled trajectories), not RL's reward-driven training. The one place RL could legitimately enter Hunter's picture is *later*, as a Phase 6+ enhancement layered on top of the already-fine-tuned LLM (optimizing its tool-use policy against real outcome data — was querying `get_process_state` before concluding actually the right call) — that's a refinement of Hunter's existing LLM, not a replacement of it with a separate RL policy the way Jury has one.
...
- **What it does**: implements the attack-lab scenarios (isolated VM only, per the README's explicit warning) and the dataset pipeline that captures real `kb-core` eBPF events during those runs, labels them (attack/benign + category), and formats them two ways — RL-episode-shaped (state/action/reward sequences, for Phase 1) and **agentic-trajectory-shaped** (thought/tool-call/observation sequences ending in a final assessment, for Phase 3's fine-tuning — corrected from the original flat prompt/completion plan; see "Hunter's training data" above).
- **Acceptance criteria**: a labeled dataset covering all six attack scenarios plus a benign baseline, split train/validation/test, in both output formats — the trajectory format must include realistic intermediate tool-call/observation steps against `ControlPlaneClient`'s actual methods (`get_process_state`, `list_zone`, `stream_events`, `stream_alerts`), not just a final answer.
...
## Phase 3 — Agentic QLoRA fine-tuning pipeline
...
- **Depends on**: Phase 0's trajectory-format dataset.
- **What it does**: fine-tunes a base model on the labeled agentic trajectories (thought → tool-call → observation → ... → final assessment), using the already-specified stack (HF Transformers + PEFT/QLoRA + bitsandbytes + TRL) — TRL's trajectory/conversation-format training, not single-turn instruction pairs, since the model needs to learn *when to call a tool* and *how to use the observation*, not just produce a final narrative.
- **Base model selection now has a real criterion it didn't have before**: `marl/README.md`'s three candidates (Phi-3 Mini / Qwen2.5 3B / Mistral 7B) were picked with no stated tool-use requirement. Now that Hunter is agentic, native function-calling/tool-use capability should be an explicit selection factor, not an afterthought — worth Karthik's own evaluation rather than assuming any of the three handles tool loops equally well.
- **Acceptance criteria**: a fine-tuned checkpoint that, given a partial investigation state, correctly decides whether to call a tool or conclude, produces well-formed tool calls against the real `ControlPlaneClient` methods, and — once it concludes — produces a threat assessment matching the dataset's format (confidence score, IOC pattern name, recommended action) at an accuracy bar Karthik defines.
...
- **What it does**: replaces the current stubs with real inference:
  ...
  - `HunterAgent.investigate()`: load the fine-tuned LLM and run an actual **tool-call loop**, not a single inference call — the model reasons, optionally calls one of `ControlPlaneClient`'s methods (`get_process_state`, `list_zone`, `stream_events`, `stream_alerts`), receives the result as an observation, and repeats until it concludes with a confidence score + evidence chain to hand to the Judge.
- **Tool-call iteration cap — proposed, not yet Karthik-confirmed**: **5 tool calls**, roughly one call of headroom over Hunter's 4-tool surface (a thorough single-PID investigation plausibly touches most tools once, maybe one twice). The dominant per-iteration cost is LLM inference on the QLoRA-quantized model, not the UDS gRPC round-trip itself, so the cap is really bounding "small multiple of one inference pass," which is the actual thing that needs to stay affordable given Hunter's per-zone-transition (not per-tick) trigger. **On hitting the cap without a natural conclusion**: force a final assessment from whatever evidence was gathered, but mark it explicitly (e.g. `incomplete_investigation: true` + a reduced confidence score) so Judge/Jury weight a forced conclusion differently from a naturally-reached one — silent failure or infinite looping are both wrong. This number is a Phase 4 starting point, not a measured one; revisit once Phase 0's real trajectory data and Phase 3's actual fine-tuned model exist to calibrate against, and treat a high cap-out rate in Phase 6 production data as a signal the cap (or the tool surface, or the base model) needs revisiting.
- **Function-calling format — proposed, not yet Karthik-confirmed**: use the fine-tuned model's **native function-calling format**, not a custom ReAct text protocol, via HF Transformers' `apply_chat_template(..., tools=...)` (already part of the specified fine-tuning stack). Reasoning: (1) Phase 0's dataset will be attack-lab-scale, not internet-scale — a QLoRA model adapting an existing pretrained tool-use convention needs far less data than one learning a bespoke protocol from scratch; (2) the four tools' arguments already have schemas — `kb.proto`'s `PidRequest`/`ZoneRequest`/`EventFilter` messages convert mechanically into the JSON-schema tool definitions native function-calling expects, keeping the tool interface in sync with the wire contract instead of hand-authored and drifting; (3) structured/schema-constrained output is more reliably parseable than free-text `Action: tool(args)` lines, especially from a small quantized model. **Tradeoff**: this ties fine-tuning data formatting and inference-time prompting to whichever base model is chosen, unlike a portable custom protocol — which sharpens (doesn't settle) the open base-model question: Qwen2.5's family has comparatively strong, well-documented native tool-calling support relative to Phi-3 Mini / Mistral 7B, making it the current front-runner on this specific axis, pending Karthik's actual evaluation.
- **Acceptance criteria**: a live single-node swarm run (same harness used to verify the scaffolding this session) where Jury votes and Hunter investigations are traceably produced by the trained artifacts, not hardcoded logic — including at least one investigation that visibly makes a real tool call against `ControlPlaneClient` mid-investigation, not just a single-shot completion, and at least one forced-conclusion case exercising the iteration cap's fallback path.
...
7. **Resolved**: Hunter's fine-tuning target is agentic (multi-turn tool-call loop), not flat prompt/completion — confirmed by Karthik. **Proposed (not yet Karthik-confirmed)**: a 5-call iteration cap with forced-conclusion-plus-reduced-confidence fallback on hit, and native function-calling over `apply_chat_template(..., tools=...)` rather than a custom ReAct text protocol — both written up under Phase 4 above, with reasoning. Still genuinely open: the explicit base-model evaluation for tool-use capability — the native-format choice sharpens this toward Qwen2.5 as a likely front-runner, but doesn't settle it; Karthik needs to actually run the evaluation, not take this doc's inference as sufficient.
...
- **2026-07-24**: Corrected the "architecture is actually specified, not open" framing — `git log --follow` confirms `marl/README.md` and `scripts/dataset/README.md` have exactly one commit each, from initial project setup (2026-06-25), never revisited. RL-vs-LLM split still holds, but Hunter's training data shape didn't survive scrutiny: flat prompt/completion pairs don't cover the multi-step investigative process `HunterAgent.investigate()`'s own TODOs describe. Resolved per Karthik: Hunter is genuinely agentic (tool-call loop against `ControlPlaneClient`'s existing methods), not single-shot. Updated Phase 0 (trajectory-format data, not flat pairs), Phase 3 (trajectory fine-tuning, tool-use as a base-model selection criterion), and Phase 4 (real tool-call loop with a hard iteration cap) accordingly. Added open question 7 for the remaining unresolved pieces.
- **2026-07-24**: Added a terminology note distinguishing RL-vs-LLM (what decides) from agentic (how the LLM operates at inference) — Hunter being agentic doesn't move it into the RL column; it's still supervised-fine-tuned, with RL only a possible Phase 6+ refinement layered on top, not a replacement paradigm.
- **2026-07-24**: Proposed (not yet Karthik-confirmed) a concrete tool-call iteration cap for Hunter — 5 calls, sized against the 4-tool surface and the LLM-inference-dominated per-iteration cost, with a forced-conclusion-plus-reduced-confidence fallback on cap-out rather than silent failure or infinite looping. Written into Phase 4; open question 7 updated to reflect this as proposed, with the function-calling-format and base-model-evaluation pieces still genuinely open.
- **2026-07-24**: Resolved the function-calling-format question (proposed, not yet Karthik-confirmed) — native function-calling via `apply_chat_template(..., tools=...)`, not a custom ReAct text protocol, reasoned from data efficiency on a small attack-lab-scale dataset, mechanical schema derivation from `kb.proto`, and parse reliability. Sharpens (doesn't settle) the base-model question toward Qwen2.5 as a likely front-runner on tool-use support specifically.
