...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=black,
  pdftitle={Node Out-of-Band Module for Attestation \& Defense (NOMAD) -- Kernel Borderlands},
...
\fancyhead[R]{\small\scshape Node Out-of-Band Module for Attestation \& Defense}
...
\tikzset{
  ...
  nomadblock/.style={block, fill=gray!17},
...
{\fontsize{34}{40}\selectfont\bfseries NOMAD}

\vspace{0.6em}

{\Large\bfseries Node Out-of-Band Module\\[0.15em] for Attestation \& Defense}
...
A complete, illustrated walkthrough of NOMAD: why Kernel Borderlands needs a
...
% =====================================================================
\section{What Is NOMAD?}
\label{sec:what-is-nomad}
...
The \textbf{Node Out-of-Band Module for Attestation \& Defense (NOMAD)} is Kernel Borderlands'
...
telling the truth?''} If the answer stops making sense, NOMAD can cut the
machine's power itself, without needing any cooperation from the
machine it is cutting off.

\begin{statusbox}
\textbf{Where NOMAD stands today.} NOMAD is a design, not yet a shipped
...
hallway. NOMAD is a second guard station across the street, watching the
...
the problem exists (\S\ref{sec:problem}), the four design rules NOMAD
...
% =====================================================================
\section{The Problem NOMAD Solves}
\label{sec:problem}
...
built into the org chart. NOMAD is what happens when you hire an
...
NOMAD's design goal is simple to state: move the very last line of
...
on that host's kernel being honest about anything. NOMAD does not
...
NOMAD applies that same principle one layer further down, at the
\emph{hardware} layer.
...
out of road, and exactly where NOMAD picks up.
...
\begin{tikzpicture}[font=\scriptsize, node distance=0.42cm and 0.4cm]
  ...
  \node[nomadblock, right=of t2, minimum width=3.1cm] (t3) {next heartbeat\\\scriptsize NOMAD attestation fails to verify};
  \node[nomadblock, right=of t3, minimum width=2.6cm] (t4) {timeout\\\scriptsize STM32 fences the host};
  ...
  \node[groupbox, fit=(t3)(t4), inner sep=0.3cm, label={[font=\tiny\bfseries]below:NOMAD's independent hardware}] (right) {};
\end{tikzpicture}%
}
...
\subsection{Where NOMAD Picks Up}

None of the four bullets above touch NOMAD, for a simple structural
reason: NOMAD never asked the kernel a question that the kernel could
...
That gap is the entire reason NOMAD exists.

% =====================================================================
\section{Design Philosophy}
\label{sec:philosophy}

Everything about NOMAD follows from four rules. They deliberately mirror
...
\midrule
Out-of-Band & NOMAD never depends on the monitored operating system for
...
loses network, or is compromised, none of that touches NOMAD. \\
...
compromised kernel. \\
Fail-Safe (configurable) & NOMAD can take protective action even when
...
different angles: \emph{``what happens to NOMAD's trustworthiness if the
host is fully owned by an attacker?''} The answer, by design, is
nothing --- NOMAD keeps working exactly as before, because nothing about
...
At the top level, NOMAD sits beside the existing Kernel Borderlands host
...
\begin{tikzpicture}[node distance=0.6cm and 1.7cm, font=\footnotesize]
  ...
  % NOMAD row 1: STM32 - SBC - ESP32
  \node[nomadblock, below=3.0cm of cp, minimum width=2.4cm] (sbc) {SBC\\\scriptsize attestation engine};
  \node[nomadblock, left=1.0cm of sbc, minimum width=1.9cm] (stm) {STM32\\\scriptsize safety controller};
  \node[nomadblock, right=1.0cm of sbc, minimum width=1.9cm] (esp) {ESP32\\\scriptsize comms processor};

  % NOMAD row 2: TPM - Display, below row 1
  \node[nomadblock, below=0.9cm of stm, minimum width=1.9cm] (tpm) {TPM 2.0\\\scriptsize root of trust};
  \node[nomadblock, right=0.6cm of tpm, minimum width=1.9cm] (disp) {Display};
  ...
  \node[groupbox, fit=(stm)(sbc)(esp)(tpm)(disp), inner sep=0.35cm] (nomadbox) {};
  \node[font=\small\bfseries] at ($(nomadbox.north)+(0,0.3)$) {NOMAD -- Node Out-of-Band Module for Attestation \& Defense};
...
\figcap{NOMAD sits beside the host, talking only to \texttt{kb-checker} over a dedicated out-of-band link. The relay gives NOMAD a physical, software-independent way to cut the host's power.}
...
\begin{itemize}
  ...
  \item \textbf{The relay is driven by the STM32, not the SBC.} Fencing
    survives a full Linux crash on NOMAD's own SBC, because the chip that
...
NOMAD exposes two communication channels that never share a wire, a
switch, or a protocol stack. Keeping them apart is what lets NOMAD scale
...
\begin{tikzpicture}[font=\footnotesize, node distance=1.1cm]
  ...
  \node[actor, right=3.4cm of kbc, minimum width=3.4cm] (sbc) {NOMAD SBC};
...
signed summaries that NOMAD's own trust plane has already vouched for
...
NOMAD's brain is deliberately split across three separate chips, each
...
This is the only chip in NOMAD complex enough to run a full Linux
...
same-failure-domain problem NOMAD exists to solve for the host in the
first place.
...
tamper with from software alone. NOMAD's TPM does double duty:

\begin{itemize}
  \item \textbf{Seals NOMAD's own integrity state.} Measured boot on the
    SBC and STM32 firmware is sealed against the TPM, so NOMAD can prove
...
that would otherwise exist: it is not enough for NOMAD to trust the host
--- NOMAD's own word has to be worth something too. A TPM that only
checked the host, sitting inside a NOMAD whose own integrity nobody
...
% =====================================================================
\section{What NOMAD Does, Continuously}

Once running, NOMAD's loop never stops:
...
\begin{tikzpicture}[font=\scriptsize]
  ...
  \node[actor] (tpm) at (3.2,\yTop) {TPM 2.0\\\scriptsize (NOMAD)};
  \node[actor] (sbc) at (6.4,\yTop) {NOMAD SBC};
...
% =====================================================================
\section{Where NOMAD Sits in the Trust Boundary}

Zooming out, NOMAD's place in the overall Kernel Borderlands data flow is
a single, sharp line:
...
\begin{tikzpicture}[font=\footnotesize, node distance=1.0cm]
  ...
  \node[nomadblock, right=1.3cm of boundary] (nomad) {NOMAD};
  \draw[darr] (kbc) -- (boundary.west);
  \draw[darr] (boundary.east) -- (nomad);
  ...
  \node[below=0.55cm of nomad, font=\tiny, align=center, text width=3.0cm] {remains\\independent};
\end{tikzpicture}
\figcap{Everything to the left of the trust boundary shares one kernel, and so shares one failure mode. NOMAD is the only component structurally guaranteed to still be telling the truth on the right.}
\label{fig:boundary}
\end{figure}
...
drop). NOMAD owns only the final stage.
...
\begin{tikzpicture}[font=\footnotesize, node distance=0.55cm]
  ...
  \node[nomadblock, below=of s4, minimum width=8.4cm, very thick] (s5) {Stage 5 -- Hardware fencing (requires NOMAD)\\\scriptsize relay cutoff -- power or reset};
  ...
  \draw[arr] (s4) -- (s5);
\end{tikzpicture}
\figcap{The recovery ladder. Stages 1--4 are entirely within \texttt{kb-checker}'s existing software authority; only Stage 5 needs NOMAD, and only after every stage above it has already failed.}
\label{fig:ladder}
\end{figure}

Because Stage 5 sits strictly below the existing chain, a healthy host
never reaches NOMAD's relay at all --- the four software stages above it
...
A single NOMAD unit protects a single host. Most real deployments protect
more than one machine, so NOMAD's reference architecture is a
...
\begin{tikzpicture}[font=\scriptsize, node distance=0.5cm and 0.6cm]
  ...
  \node[nomadblock, minimum width=2.0cm] (sbc) {SBC\\shared};
  \node[nomadblock, right=of sbc, minimum width=2.0cm] (esp) {ESP32\\shared};
  \node[nomadblock, right=of esp, minimum width=2.0cm] (tpm) {TPM\\shared};
  \node[nomadblock, right=of tpm, minimum width=2.0cm] (disp) {Display\\shared};
  ...
  \node[groupbox, fit=(sbc)(esp)(tpm)(disp)(c1)(c2)(c3)(c5), inner sep=0.35cm, label={[font=\small\bfseries]above:Shared NOMAD chassis -- max 5 hosts}] (chassis) {};
...
local NOMAD attestation, local recovery, and local fencing --- nothing
...
\begin{tikzpicture}[font=\footnotesize, node distance=0.5cm]
  ...
  \node[nomadblock, below=of c, minimum width=7.6cm] (n) {NO loss of local, per-host protection};
  \draw[arr] (f) -- (c);
  \draw[arr] (c) -- (n);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\footnotesize, node distance=0.9cm]
  ...
  \node[hostblock, right=of core] (cp) {kb-control-plane};
  \node[hostblock, right=of cp] (kbc) {kb-checker};
  \node[nomadblock, right=of kbc] (sum) {Signed summary};
  \node[nomadblock, right=of sum] (fms) {FMS};
  \draw[arr] (core) -- (cp);
  \draw[arr] (cp) -- (kbc);
  \draw[arr] (kbc) -- (sum);
  \draw[arr] (sum) -- (fms);
...
both NOMAD's own state and the host's attestation (\S\ref{sec:tpm-relay}),
...
Health Monitoring & Host availability, resource usage, \texttt{kb-core}/\texttt{kb-checker}/NOMAD connectivity, heartbeat. \\
...
\begin{tikzpicture}[font=\footnotesize, node distance=0.5cm]
  ...
  \node[nomadblock, below=of f, minimum width=4.6cm] (n) {Node Policy -- can override inherited defaults};
  \draw[arr] (g) -- (c);
  \draw[arr] (c) -- (f);
  \draw[arr] (f) -- (n);
\end{tikzpicture}
...
layer: each Colony keeps its own NOMAD chassis and its own FMS instance,
with no global master coordinating between them. A Colony stays
autonomous even if NOMADs are later given a way to synchronize with each
...
NOMAD's architecture or its guarantees.
...
\begin{itemize}
  \item \textbf{The discrete TPM.} It is the one component whose entire
    ...
    firmware-TPM would put NOMAD's own root of trust on the same silicon
    as its general-purpose SBC --- reintroducing, inside NOMAD itself,
    the exact same-failure-domain problem NOMAD exists to solve for the
    host.
...
Beyond the bench-buildable chassis, NOMAD's production form factor is a
...
Every component in NOMAD has exactly one job, and no component both makes
...
TPM & Hardware trust anchor for both NOMAD and the host \\
...
That separation is what lets NOMAD make a claim no purely-software
...
makes both halves explicit: what NOMAD guarantees, and what it
deliberately leaves to other parts of Kernel Borderlands.

\subsection{What NOMAD Guarantees}
...
reach NOMAD -- there is no code path for a compromised kernel to
intercept. \\
...
(\S\ref{sec:stm32}). \\
Tamper-evidence of NOMAD's own state & Sealed against the TPM, covering
both NOMAD's own boot state and the host's reported measurements
(\S\ref{sec:tpm-relay}). \\
...
\begin{itemize}
  ...
  \item \textbf{It does not protect against physical access to itself.}
    ...
    outside NOMAD's own scope.
  \item \textbf{It does not eliminate the need for the four software
    recovery stages.} NOMAD is Stage 5 of five (\S\ref{sec:recovery}) --
    the overwhelming majority of real incidents are expected to resolve
    at Stages 1--4, long before NOMAD's relay is ever involved.
...
\begin{plainbox}
A useful one-line summary: NOMAD answers exactly one question --
...
to the witness before they walked in. NOMAD's heartbeat
...
\begin{tikzpicture}[node distance=0.6cm and 1.3cm, font=\footnotesize]
  ...
  \node[draw=black!60, dashed, rounded corners=2pt, fill=gray!3, below=0.6cm of silent,
    ...
    {\textbf{What NOMAD sees:} no heartbeat arrives. Timer expires, fail-closed policy fires -- \S\ref{sec:heartbeat}. \textbf{Well covered.}};
  \node[draw=black!60, dashed, rounded corners=2pt, fill=gray!3, below=0.6cm of lying,
    ...
    {\textbf{What NOMAD sees, today:} a clean, on-time, validly-signed attestation. \textbf{Not covered yet.}};
...
own} TPM 2.0 -- physically on the protected machine, not NOMAD's. A TPM
can only make a hardware-backed statement about the machine it is
part of; NOMAD's own TPM (\S\ref{sec:tpm-relay}) can seal NOMAD's own
...
hardware, and no tension with \texttt{nomad-cheap.md} \S3.7's rejection
of fTPM: that argument is specifically about not substituting the
host's fTPM for \emph{NOMAD's own} independent trust anchor. Using the
...
exactly what \texttt{nomad-cheap.md} itself calls ``genuinely free.''
\end{notebox}
...
\begin{tikzpicture}[node distance=0.5cm and 0.6cm, font=\footnotesize]
  ...
  \node[nomadblock, below=0.7cm of kbc2, minimum width=3.2cm] (tpm2) {TPM 2.0 (host)\\\scriptsize signs on request only};
...
\begin{tikzpicture}[node distance=0.5cm and 0.5cm, font=\footnotesize]
  ...
  \node[nomadblock, below=1.6cm of chain, minimum width=5.4cm] (sealed) {Key sealed at boot: usable only if\\PCR digest = this known-clean value};
...
\begin{itemize}
  ...
  \item \textbf{Upstream compromise of the measurement chain itself}
    ...
    threat class this doesn't address -- and isn't really NOMAD's problem
    to solve; it's the layer NOMAD's own trust starts from.
...
\begin{tikzpicture}[node distance=1.35cm, font=\footnotesize]
  ...
  \node[nomadblock, below=of b4, minimum width=7.6cm, minimum height=1.1cm] (b5) {Memory-corruption-only, already-measured code, no info leak, evades \S\ref{sec:cross-consistency}--\S\ref{sec:clock-crosscheck}};
...
place that gets to take a disruptive action stays NOMAD's own
...
\begin{tikzpicture}[node distance=0.5cm and 0.9cm, font=\footnotesize]
  ...
  \node[nomadblock, below=1.0cm of core, xshift=1.8cm, minimum width=3.6cm] (cmp) {Compare -- \texttt{kbd} or NOMAD's SBC};
  ...
  \node[nomadblock, below=0.8cm of syscall, xshift=1.55cm, minimum width=3.2cm] (cmp2) {Compare};
...
single comparison point (\texttt{kbd}, or NOMAD's SBC over the OOB link)
...
in the signed payload, and have NOMAD's SBC track the \emph{distribution}
...
NOMAD's SBC has its own independent wall clock by construction -- it is
...
happened,'' and a report that just says \emph{NOMAD tripped} is barely
...
That classification is the difference between ``NOMAD tripped'' and
``NOMAD tripped, category: cryptographic failure, consistent with an
...
% =====================================================================
\section{Reprogramming Resistance: Protecting NOMAD's Own Hardware}
\label{sec:reprogram}

\S\ref{sec:properties} already states one of NOMAD's honest limits
plainly: it does not protect against physical access to itself. That
line has stood as a scope boundary, not a design. This section narrows
it as far as it currently goes: if an attacker can reach NOMAD's own
SBC, STM32, or ESP32 directly, what -- if anything -- stops them from
simply reprogramming it?

\begin{notebox}[Status]
Nothing in this section is built or decided. It is a design direction,
offered at the same level of maturity as \S\ref{sec:whatif}'s own open
items, and it deliberately does not resolve NOMAD's own internal
SBC$\leftrightarrow$STM32$\leftrightarrow$ESP32 protocol, which remains
its own separate, undesigned question.
\end{notebox}

\subsection{The Question, Stated Precisely}

Everything in \S\ref{sec:three-chips}--\S\ref{sec:tpm-relay} assumes
each chip is running the firmware it was built with. Nothing described
so far actually enforces that assumption against an attacker who can
reach the hardware directly -- pull the SBC's storage, attach a
debugger to the STM32's SWD pins, or push new firmware onto the ESP32.
None of that requires defeating NOMAD's attestation logic at all; it
just replaces the logic.

\begin{plainbox}
A locked door is only as good as the frame it's mounted in. Everything
this document has described so far assumes NOMAD's own three chips are
running the firmware they were built with. This section is about the
frame: what stops someone from simply replacing that firmware if they
can get their hands on the board.
\end{plainbox}

\subsection{A Lockdown Mechanism for Each Chip}
\label{sec:reprogram-mechanisms}

Each of NOMAD's three processors (\S\ref{sec:three-chips}) already has
a standard, vendor-supported way to close this, at zero new hardware
cost -- the same ``already-shipping, zero-new-hardware'' property every
mitigation in \S\ref{sec:whatif} insists on.

\subsubsection{STM32 --- Readout Protection Level 2}

STM32 parts expose Readout Protection (RDP) as a hardware option
byte~\cite{stm32rdp}. RDP Level 1 blocks debug read access to flash but
still permits a mass-erase-and-reflash; RDP Level 2 permanently and
irreversibly disables the SWD/JTAG debug interface -- no external
access to the chip at all, ever again. Lines with TrustZone-M
(STM32L5/U5/H5) ship a reference Secure Boot/Secure Firmware Update
stack: with RDP2 set, the only way to load new firmware afterward is
through a bootloader that checks a signature against a key burned in
at provisioning time.

\begin{notebox}[One-Way Door]
RDP Level 2 is irreversible by design. The signed-update bootloader
path has to be built and tested \emph{before} RDP2 is set on a
production unit, or a bricked STM32 is unrecoverable -- which is also,
from a security standpoint, the entire point.
\end{notebox}

\subsubsection{ESP32 --- Secure Boot and Flash Encryption}

The ESP32 (\S\ref{sec:three-chips}) supports Secure Boot V2 and Flash
Encryption~\cite{esp32secboot}, both gated by one-time-programmable
eFuses: only firmware signed against a key digest burned into those
eFuses will boot, and the same eFuses can permanently disable the
chip's JTAG interface. Because the ESP32 already carries the Fabric
Management Plane (\S\ref{sec:planes}) -- ordinary Ethernet, the one
channel with any real network exposure -- this is the chip where
reprogramming lockdown matters most against a \emph{remote} attacker,
not only a physical one.

\subsubsection{SBC --- Signed Boot and a Read-Only Root Filesystem}

Both of \S\ref{sec:sbc}'s named SBC candidates support this. A
Raspberry Pi 4/5 can burn a public-key hash into its SoC's
one-time-programmable region~\cite{rpisecboot}, after which only a
correspondingly-signed bootloader/OS image boots at all. An Intel N100
already ships UEFI Secure Boot and Intel Boot Guard~\cite{intelbootguard}
as standard platform features -- no fuse-burning step required. Either
can be paired with a read-only root filesystem (\texttt{dm-verity}) for
the running OS, so even a live compromise cannot persist a
modification to disk.

\subsubsection{Tying It to NOMAD's Own TPM}

\S\ref{sec:tpm-relay} already establishes that NOMAD's own TPM seals
measured boot on the SBC and STM32. The natural extension is to make
that seal do real work here: gate a firmware-\emph{update}
authorization key behind the same \texttt{TPM2\_PolicyPCR} mechanism
\S\ref{sec:pcr-seal} already uses for the host's key, so an update can
only proceed while NOMAD's own measured state matches its
last-known-good value -- not just ``is this image correctly signed,''
but ``is this device currently in the state it should be in before I
let it change.'' That is a stronger, self-terminating property than a
plain signed-bootloader check on its own, using a chip already in the
design.

\subsection{How Much of the Reprogramming Threat This Actually Closes}
\label{sec:reprogram-analysis}

\begin{center}
\renewcommand{\arraystretch}{1.5}
\small
\begin{tabular}{@{}p{2.6cm}p{6.7cm}p{4.3cm}@{}}
\toprule
\textbf{Chip} & \textbf{Physical reprogramming (open the case)} & \textbf{Remote reprogramming} \\
\midrule
STM32 & Closed by RDP2 -- no debug access, no path without the signing
key. & Already near-zero: the STM32 has no network stack of its own
(\S\ref{sec:stm32}). \\
ESP32 & Closed by eFuse-locked JTAG + Secure Boot. & Narrowed, not
closed: a live RCE against its Ethernet/FMS stack (\S\ref{sec:planes})
cannot \emph{persist} past a reboot, but is not prevented while power
stays on. \\
SBC & Closed by OTP-signed boot (Pi) or UEFI Secure Boot + Boot Guard
(N100) -- a stock SD-card swap no longer works. & Narrowed: the SBC's
only external link is the point-to-point OOB Trust Plane
(\S\ref{sec:planes}), already a small surface; signed boot mainly
denies persistence to whatever does get through it. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{notebox}[What This Does Not Close]
\begin{itemize}
  \item \textbf{Live, non-persistent compromise.} None of the three
    mechanisms above stop an exploit that never tries to write new,
    unsigned code to persistent storage -- the same class of gap
    \S\ref{sec:memcorr-narrowing} already names for the host, present
    here too. \S\ref{sec:reprogram-memcorr} below narrows it, using the
    same playbook, but does not close it.
  \item \textbf{The still-open inter-chip trust question.} Proving the
    SBC's \emph{own} firmware is unmodified says nothing about whether
    the verdict it hands the STM32 is being interpreted correctly. That
    is a distinct, still-undesigned protocol question -- reprogramming
    lockdown and inter-chip trust are two separate problems, and
    closing the first does not get the second for free.
  \item \textbf{Fault-injection-class physical attacks.} RDP-style
    protections are not absolute -- published research has defeated
    microcontroller readout protection, including STM32 parts, via
    voltage/clock glitching during the check itself~\cite{obermaier2021}.
    This raises the bar from a low-cost debugger to dedicated
    fault-injection equipment; it does not raise it to zero.
  \item \textbf{Supply-chain integrity.} If a signing key is
    compromised, or a malicious image is signed before a unit ever
    ships, every mechanism above enforces exactly the wrong thing -- it
    protects the backdoor as if it were legitimate. A genuinely
    separate concern from the reprogramming threat this section
    addresses, and not covered here.
\end{itemize}
\end{notebox}

\subsection{Narrowing the Same Gap on NOMAD's Own Chips}
\label{sec:reprogram-memcorr}

\S\ref{sec:memcorr-narrowing} lays out a full narrowing playbook for
memory-corruption exploitation on the \emph{host}. The same gap exists
on NOMAD's own three processors, and the same principle applies --
though not always the same mechanism, since an SBC running Linux and a
bare-metal STM32 firmware image are not the same kind of system.
...
\toprule
\textbf{Chip} & \textbf{What applies, and why} \\
\midrule
SBC & Direct reuse, no adaptation: it is a real Linux system, so
\S\ref{sec:memcorr-narrowing}'s entire host playbook -- ASLR/KASLR +
KPTI, \texttt{STRICT\_KERNEL\_RWX}, stack canaries,
\texttt{kexec}/\texttt{devmem} lockdown, LKRG in report-only mode --
applies to NOMAD's own SBC image exactly as written, with the same
residual sliver: a ROP/JOP or data-only exploit against already-loaded
code. \\
STM32 & The principle transfers, the mechanism does not: there is no
dynamic loader and no meaningful address space to randomize, so
classic ASLR buys little. What does apply: stack canaries
(\texttt{-fstack-protector}, standard on embedded GCC), and RAM marked
execute-never in the Cortex-M's MPU -- the direct MCU analog of
\texttt{STRICT\_KERNEL\_RWX}, forcing any successful corruption into
ROP against a firmware image small enough that gadget availability is
itself limited. \\
ESP32 & A middle case: ESP-IDF ships stack-smashing and
heap-corruption protection by default, and the RISC-V variants (S3/C3)
have PMP (Physical Memory Protection) capable of the same
execute-never trick STM32's MPU provides. This chip carries the most
complex, most attackable stack of the three (Ethernet handling for the
Fabric Management Plane), so it is also where this narrowing matters
most. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{plainbox}
Nothing here claims the memory-corruption gap is closed on any of
NOMAD's own three chips -- it isn't, on any of them, for the same
reason it isn't closed on the host. What changes is that it stops
being a flat, unaddressed line item and becomes the same kind of
narrowed, specific residual \S\ref{sec:memcorr-narrowing} already
accepts for the host: a control-flow-hijack-only exploit against code
that is already loaded, with no new code ever written to persistent
storage.
\end{plainbox}

\subsection{Where This Leaves \S\ref{sec:properties}'s Stated Limit}

\S\ref{sec:properties} states, plainly, that NOMAD does not protect
against physical access to itself. This section does not overturn that
-- physical access to an opened chassis, given enough specialized
equipment, still is not fully ruled out by any mechanism above. What it
does is turn a blanket limitation into a specific, per-chip one: a
stock ST-Link and a screwdriver -- the realistic threat for most
deployments -- no longer works against any of the three chips; a
fault-injection lab might, against the STM32 specifically. That is a
narrower, more honestly-scoped version of the same stated limit, not a
claim that the limit has been removed.

% =====================================================================
\section{How NOMAD Compares to Existing Approaches}
\label{sec:comparison}

NOMAD's shape -- an independent management processor, a hardware root of
...
\toprule
\textbf{Approach} & \textbf{What it shares with NOMAD} & \textbf{Where NOMAD differs} \\
\midrule
...
one server for remote administration; NOMAD's primary purpose is
...
resistance. & An HSM protects keys for applications to use; NOMAD's TPM
...
designs stop at detection; NOMAD adds a deterministic, host-independent
...
The honest framing is that NOMAD combines ideas each of these fields has
...
\item[Can an attacker just unplug NOMAD?] \hfill \\
...
\item[What stops someone from swapping NOMAD's SBC for a fake one that
...
\item[Does NOMAD need to trust the network it runs on?] \hfill \\
...
\item[Is NOMAD required to use Kernel Borderlands at all?] \hfill \\
No. Every host keeps full protection from \texttt{kb-core},
\texttt{kb-control-plane}, and \texttt{kb-checker} with no NOMAD present
at all. NOMAD adds a hardware trust anchor on top of an already-complete
software stack; it is a hardening layer, not a dependency.

\end{description}

% =====================================================================
\section{Positioning: What NOMAD Adds to Kernel Borderlands}

NOMAD extends Kernel Borderlands beyond a software-only runtime defense
...
\begin{tikzpicture}[font=\scriptsize, node distance=0.55cm and 0.7cm]
  ...
  \node[nomadblock, right=of p1, minimum width=4.3cm, minimum height=1.3cm] (p2)
...
\begin{description}[leftmargin=3.2cm, style=nextline, itemsep=0.35em]
  ...
  \item[BMC] Baseboard Management Controller -- the enterprise-server
    equivalent of NOMAD's role: a small, independent management processor
...
\begin{itemize}[label=$\square$]
  \item Confirm the \emph{host's own} TPM 2.0 is present and enabled in
    ...
    from NOMAD's own TPM and does the job described in
...
\midrule
\endhead
What NOMAD is & A physically separate hardware appliance that attests to
...
Its root of trust & A discrete TPM 2.0, sealing both NOMAD's own state
...
\path{docs/features/hardware-ext/} \newline \path{nomad-hardware-appliance.md} \\[0.3em]
Fleet scaling, Fabric $\to$ Colony $\to$ Family $\to$ Node, FMS
responsibilities &
\path{docs/features/hardware-ext/nomad-fms.md} \\[0.3em]
Budget build, cost-reduction levers &
\path{docs/features/hardware-ext/nomad-cheap.md} \\[0.3em]
Boot/rootkit compromise resistance, PCR-sealing, this section's markdown twin &
\path{docs/features/hardware-ext/} \newline \path{nomad-rootkit-resistance.md} \\[0.3em]
...
multiple Colonies), one Colony's shared NOMAD chassis, its per-host
...
TPM (shared) & NOMAD's own root of trust (\S\ref{sec:tpm-relay}) -- seals NOMAD's own integrity and verifies what \texttt{kb-checker} reports. Distinct from any host's own TPM (\S\ref{sec:pcr-seal}). \\
...
\texttt{kb-checker} & The existing software watchdog -- watches the rest of the stack independently, and is the one component that also feeds the OOB Trust Plane up to NOMAD's SBC (Figure~2). \\
...
\begin{tikzpicture}[font=\scriptsize, node distance=0.5cm and 0.6cm]
  ...
  \node[font=\small\bfseries] at ($(chassis.north)+(0,0.35)$) {One Colony -- shared NOMAD chassis, max 5 hosts (\S13)};

  % ---- Fleet layer, as a compact side branch off the chassis (reads on
  % the side, not stacked above -- keeps the tight dimension short) ----
  \node[nomadblock, left=1.4cm of chassis, minimum width=2.8cm, yshift=0.9cm] (fabric)
...
\bibitem{stm32rdp} STMicroelectronics, \emph{STM32 Microcontroller Readout Protection (RDP) and Option Bytes}, reference manual documentation, used in \S\ref{sec:reprogram-mechanisms}.

\bibitem{esp32secboot} Espressif Systems, \emph{Secure Boot V2 and Flash Encryption}, ESP-IDF Programming Guide. \url{https://docs.espressif.com}.

\bibitem{rpisecboot} Raspberry Pi Trading Ltd., \emph{Bootloader Configuration --- Secure Boot}, Raspberry Pi Documentation. \url{https://www.raspberrypi.com/documentation/}.

\bibitem{intelbootguard} Intel Corporation, \emph{Intel Trusted Execution Technology and Boot Guard}, platform security documentation.

\bibitem{obermaier2021} J.~Obermaier, M.~Schink, and K.~Moczek, ``One Bit to Rule Them All: Bypassing Microcontroller Readout Protection through Fault Injection,'' in \emph{Proc.\ IEEE European Symposium on Security and Privacy Workshops (EuroS\&PW)}, 2021.
...
{\normalsize for reading through the Node Out-of-Band Module for Attestation \& Defense design}
