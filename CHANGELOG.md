...
### Added
- **KBC hardware watchdog proposal (`docs/features/kbc-hardware-watchdog.md`)**: Documentation-only proposal (not implemented) extending `kb-checker`'s existing external-watchdog role to a physically separate SBC — out-of-band link, TPM 2.0 root of trust, relay-based host fencing — to cover the one failure mode software-only same-host watchdogging can't structurally address: full kernel compromise. Scoped deliberately to `kb-checker` only (other subsystems are telemetry consumers, not independent watchers). Includes a ~$90–$180 bench-buildable bill of materials for a physical review demo; a rejected inline-hardware (FPGA/DPU) tier noted but not pursued on cost/scope grounds; open questions (wire protocol, failure semantics, TPM integration point, repo placement) explicitly left unresolved for a future implementation pass. [PardhuVarma]
